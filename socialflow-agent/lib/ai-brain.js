 ...
 */
/**
 * Phase 8 Fix 1: heuristic ad-post detector.
 * Fires BEFORE AI scoring to avoid wasting tokens on obvious promotions.
 * Signals: hashtag density, bullet/CTA emojis, links, long structured body.
 */
function detectAdPost(post) {
  const body = (post?.body || post?.text || post?.content || '').toString()
  if (!body) return false
  const hashtagCount = (body.match(/#\w+/g) || []).length
  const hasBulletPoints = /[✅✔️👉🔥💥🎯💰📌📍🚀]/.test(body) || /^\s*[-•*]\s/m.test(body)
  const hasLink = /https?:\/\/|www\.|\.com|\.vn|\.io|\.app|bit\.ly|\.net/i.test(body)
  const isLong = body.length > 500
  const hasCta = /\b(inbox|nhắn tin|liên hệ|mua ngay|đặt ngay|khuyến mãi|ưu đãi|register|sign ?up|order now)\b/i.test(body)

  // Heuristic: 3+ hashtags AND (structured body OR long) = ad
  if (hashtagCount >= 3 && (hasBulletPoints || isLong)) return true
  // 2+ hashtags + link + CTA keyword = ad
  if (hashtagCount >= 2 && hasLink && hasCta) return true
  // Very long + bullets + link = ad (even without hashtags)
  if (isLong && hasBulletPoints && hasLink) return true
  return false
}
...
${brandName ? `Thương hiệu chúng ta: ${brandName}${brandDescription ? ` — ${brandDescription}` : ''}. Bài liên quan đến ${brandName} HOẶC "${topic}" đều có relevance cao.` : ''}
